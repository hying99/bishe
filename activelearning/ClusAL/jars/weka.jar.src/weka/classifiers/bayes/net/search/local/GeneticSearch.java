...
public class GeneticSearch extends LocalScoreSearchAlgorithm {
  ...
  protected void search(BayesNet paramBayesNet, Instances paramInstances) throws Exception {
    ...
    byte b2;
    for (b2 = 0; b2 < getPopulationSize(); b2++) {
      arrayOfBayesNetRepresentation[b2] = new BayesNetRepresentation(this, paramInstances.numAttributes());
      arrayOfBayesNetRepresentation[b2].randomInit();
      if (arrayOfBayesNetRepresentation[b2].getScore() > d) {
        copyParentSets(bayesNet, paramBayesNet);
        d = arrayOfBayesNetRepresentation[b2].getScore();
      } 
    } 
    for (b2 = 0; b2 < this.m_nRuns; b2++) {
      ...
      for (byte b3 = 0; b3 < getDescendantPopulationSize(); b3++) {
        arrayOfBayesNetRepresentation1[b3] = arrayOfBayesNetRepresentation[this.m_random.nextInt(getPopulationSize())].copy();
        if (getUseMutation()) {
          if (getUseCrossOver() && this.m_random.nextBoolean()) {
            arrayOfBayesNetRepresentation1[b3].crossOver(arrayOfBayesNetRepresentation[this.m_random.nextInt(getPopulationSize())]);
          } else {
            arrayOfBayesNetRepresentation1[b3].mutate();
          } 
        } else {
          arrayOfBayesNetRepresentation1[b3].crossOver(arrayOfBayesNetRepresentation[this.m_random.nextInt(getPopulationSize())]);
        } 
        if (arrayOfBayesNetRepresentation1[b3].getScore() > d) {
          copyParentSets(bayesNet, paramBayesNet);
          d = arrayOfBayesNetRepresentation1[b3].getScore();
        } 
      } 
      boolean[] arrayOfBoolean = new boolean[getDescendantPopulationSize()];
      for (byte b4 = 0; b4 < getPopulationSize(); b4++) {
        int i = 0;
        if (this.m_bUseTournamentSelection) {
          ...
          for (byte b = 0; b < getDescendantPopulationSize(); b++) {
            if (!arrayOfBoolean[b] && arrayOfBayesNetRepresentation1[b].getScore() > d1) {
              d1 = arrayOfBayesNetRepresentation1[b].getScore();
              i = b;
            } 
          } 
        } 
        arrayOfBayesNetRepresentation[b4] = arrayOfBayesNetRepresentation1[i];
  ...
  class BayesNetRepresentation {
    ...
    void calcScore() {
      ...
      this.m_fScore = 0.0D;
      for (b = 0; b < this.m_nNodes; b++)
        this.m_fScore += this.this$0.calcNodeScore(b); 
    }
