...
aa <- 1:5
resulthm <- matrix(nrow = 7,ncol = 5)
resulthu <- matrix(nrow = 7,ncol = 5)
colnames(resulthm) <- paste("dataset",aa,sep = "")
colnames(resulthu) <- paste("dataset",aa,sep = "")
row.names(resulthm) <- c("TPR","TOP-DOWN","DOWN-TOP","DAGimprove","CLUS-HMC","HMC-DAG-SVM","HMC-DAG-MLP")
row.names(resulthu) <- c("TPR","TOP-DOWN","DOWN-TOP","DAGimprove","CLUS-HMC","HMC-DAG-SVM","HMC-DAG-MLP")
...
for (aa in 1:5)
{
  ...
  resulthm[7,aa] <- DAGlabelnnresult[1,3]
  resulthu[7,aa] <- DAGlabelnnresult[1,6]
}
