...
DownTopStepForMe<-function (go.for.level,go.leaf.nodes,nodes.to.index,nodes.to.children,prob.for.genes)
{
    ...
    {      
      {
        for(j in 1:length(go.for.level[[i]]))
        {
          ...
          if(is.leafnode==FALSE)          #若不为叶子节点
          {
            ...
            {
              if(downtop.prob[k,children.index[[m]]]>0.5)#若样本k在此子节点的判断结果为正
              {
                children.pos.num=children.pos.num+1#子节点结果为正的数量加1
                #计算正类子节点的总体贡献值
                inter.pos.prob=inter.pos.prob+downtop.prob[k,children.index[[m]]]
              }
            }
