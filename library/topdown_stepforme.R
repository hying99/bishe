TopDownStepForMe<-function (go.for.level,go.leaf.nodes,nodes.to.index,nodes.to.children,downtop.prob)
{
      ...
      for(j in 1:length(go.for.level[[i]]))
      {
        ...
        if(is.leafnode!=TRUE)
        {
          ...
          for(m in 1:length(children.index))
          {
            ...
            if(topdown.prob[k,children.index[[m]]]>inter.pos.prob)
            {
              topdown.prob[k,children.index[[m]]]=inter.pos.prob
